...
#: Logger every message from this library goes through, so an embedding
#: application can capture them instead of having them printed. Without a
#: handler configured, logging prints warnings and errors to stderr and
#: drops the rest. (HISPEC)
...
class Xeryon:
    ...
    def start(self, external_communication_thread=False, external_settings_default=None,
        ...
        Args:
            ...
            do_reset: If False, skip the per-axis reset. A reset invalidates
                the encoder index, so a process that reconnects to an
                already-referenced stage needs this off to avoid having to
                re-reference it. (HISPEC)
            send_settings: If False, load the settings file into the
        ...
        if send_settings is None:
            send_settings = AUTO_SEND_SETTINGS
...
class Communication:
    ...
    def openPort(self):
        """Open and return the port object the communication thread reads
        and writes. Overridable so a subclass can supply a different
        transport (e.g. a socket) without duplicating this class. (HISPEC)"""
        return serial.Serial(self.COM_port, self.baud, timeout=0.01)
        ...
        try:
            while self.stop_thread is False and self.ser.is_open:
                ...
                try:
                    ...
                    logger.error("%s%s", getTimestampPrefix(), e)
            ...
            self.ser.reset_output_buffer()
            self.ser.close()
            logger.info("%sCommunication has stopped.", getTimestampPrefix())
        except Exception as e:
            logger.error("%sAn error has occurred that crashed the communication thread.",
                         getTimestampPrefix())
            logger.error("%s%s", getTimestampPrefix(), e)
...
def outputConsole(message, error=False):
    """Log ``message`` if OUTPUT_TO_CONSOLE is True.

    Prefixed with a "[YYYY-MM-DD HH:MM:SS.mmm]" timestamp unless
    OUTPUT_CONSOLE_TIMESTAMPS is set to False. (HISPEC) This goes through
    the module logger rather than print(), so an embedding application can
    route it into its own log.

    Args:
        message: The text to log.
        error: If True, log at error level rather than info.
    """
